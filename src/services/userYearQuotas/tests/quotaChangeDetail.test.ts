...
const values = (vacationDays: number, homeOfficeDays: number, carriedOverDays: number) => ({
  vacationDays,
  homeOfficeDays,
  carriedOverDays,
});
...
describe("describeQuotaChange", () => {
  it("spells out every value when the member had no quota for the year", () => {
    const detail = describeQuotaChange("2026", undefined, values(25, 5, 3));

    expect(detail).toBe(
      "Quota for 2026 set to 25 vacation / 5 home office days, 3 carried over from the previous year"
    );
  });
