...
- Stored: latitude, longitude, accuracy. A session is created on click without coordinates; later
  fixes update it only while they improve accuracy, and only within a short window after the click.
- Coordinates are nulled twelve months after the business date by the retention sweep. The session
  stays.
