...
| **Approver**             | May decide member-submitted requests in a group: the manager, the main or temp approver, or a member given approver access. Org admins never are.                                                                                                                                                     |
...
| **Flagged day**          | A business date that asks for a look: a session auto-closed, clocked into on an excluded day, changed by its owner after the day, or still open once the day has passed. Avoid: problem day, error day.                                                                                               |
...
| **Location fix**         | Latitude, longitude and accuracy from the browser or the phone at one end of an attendance session. Never required, erased after twelve months. See [`docs/attendance.md`](docs/attendance.md).                                                                                                       |
...
| **Native session**       | A session created by the phone app. Bound to one Device id, valid ten years or until sign-out, a password reset, a password change made elsewhere, or a sign-in on the same phone. Avoid: mobile session, app session.                                                                                |
...
1. User creates a vacation request for a group over an inclusive date range (`from`/`to` on
   `POST /create-vacation`; a single day is `from == to`). One row is stored per working day: the
   group's non-working days and the public holidays of its holiday country are skipped.
...
forgot, through the queries and transactions in `attendance/attendanceCeilingCloses.ts` — a session
left running past its organization's ceiling, at `started_at` plus the ceiling rather than at the
instant the tick ran, and a break past its own. Once a session's close commits,
`attendance/attendanceNotifier.ts` leaves its owner an in-app notice. Neither sweep asks the plan: a
lapse must not leave a clock open forever.
