...
| **Calendar record type** | The classification of a vacation row: vacation, home office, sick, sick day, paid time off, non-paid leave, study leave, bank holiday or other. Code says `CalendarRecordType`; the PG enum keeps its historical name `vacation_type`. Avoid: leave type, vacation type, vacation kind. |
| **Requestable**          | A calendar record type a member may create via `POST /create-vacation` — every type except bank holiday, which only the bank-holiday flow writes.                                                                                                                                       |
| **Sick**                 | Absence due to illness. Unmetered. Distinct from **Sick day**.                                                                                                                                                                                                                          |
| **Sick day**             | A paid sick day taken as an organization benefit (stored `SICK_DAY`, renamed from `SICK_LEAVE`). Exists only where the organization enables it; quota-bearing.                                                                                                                          |
